...
int utils::RandomInteger(int lowerBound, int upperBound)
{
	return (rand() % (upperBound - lowerBound + 1)) + lowerBound;
}

float utils::RandomFloat(float lowerBound, float upperBound)
{
  
    const float range = upperBound - lowerBound;
    const float random = static_cast<float>(rand()) / RAND_MAX;
    return random * range + lowerBound;
}
float utils::Map(const float number, const float start1, const float stop1, const float start2, const float stop2)
{
	if(stop1-start1 == 0)
		return 0;
